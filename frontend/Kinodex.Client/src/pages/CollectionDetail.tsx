...
import { FaTrash, FaCheck, FaImage, FaEllipsisV } from "react-icons/fa";
...
function CollectionDetail() {
    ...
    try {
      ...
      if (moviesRes.ok) {
        ...
        const filtered = data.filter(
          (movie: Movie) =>
            movie.collections &&
            movie.collections.includes(collectionName || ""),
        );
        setMovies(filtered);
      }
  ...
  const completionPercentage =
    listItems.length > 0
      ? Math.round(
          (listItems.filter((item) => isMovieOwned(item.title, item.year))
            .length /
            listItems.length) *
            100,
        )
                      ...
                      {!isEditing && collection && (
                        <div ref={actionsDropdownRef} className="relative mb-2">
                          ...
                          {isActionsDropdownOpen && (
                            <div className="text-sm absolute left-0 mt-2 w-36 bg-gray-700 border border-gray-600 rounded-md shadow-lg z-10">
                              <button
                    ...
                    <div className="absolute z-10 w-full bg-gray-700 border border-gray-600 rounded-md shadow-lg max-h-96 overflow-y-auto">
                      {searchResults.map((movie) => {
                        const year = movie.release_date
                          ? new Date(movie.release_date).getFullYear()
                          : "N/A";
                        const alreadyInList = listItems.some(
                        ...
                        return (
                          <button
                            key={movie.id}
                            onClick={() =>
                              !alreadyInList && handleAddToList(movie)
                            }
                  ...
                  <h3 className="text-lg font-semibold mb-3">
                    Checklist (
                    {
                      listItems.filter((item) =>
                        isMovieOwned(item.title, item.year),
                      ).length
                    }{" "}
                    of {listItems.length})
                  </h3>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <span
                                  className={`font-medium ${owned ? "text-green-400" : "text-white"}`}
                                >
                                  {item.title}
                                </span>
                            ...
                            <button
                              onClick={() =>
                                item.id && handleRemoveFromList(item.id)
                              }
