    ...
    try {
      ...
      if (response.ok) {
        ...
        if (matchingResult) {
          setMatchingResult({
            ...matchingResult,
            suggestions: matchingResult.suggestions.filter(
              (s) => s.movieId !== movieId,
            ),
          });
        }
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div className="flex-1">
                    <h2 className="text-xl font-semibold mb-1">
                      {unmatchedMovies.length} Unmatched{" "}
                      {unmatchedMovies.length === 1 ? "Movie" : "Movies"}
                    </h2>
                    ...
                    {matching && (
                      <div className="mt-4 bg-blue-900/30 border border-blue-700 rounded-lg p-4">
                        ...
                        <p className="text-sm text-gray-400">
                          Processing {unmatchedMovies.length} movies
                          (Rate-limited to prevent API throttling)
                        </p>
                        <p className="text-sm text-gray-400 mt-1">
                          Estimated time: ~
                          {Math.ceil((unmatchedMovies.length * 0.25) / 60)}{" "}
                          minute
                          {Math.ceil((unmatchedMovies.length * 0.25) / 60) === 1
                            ? ""
                            : "s"}
                          . Please wait...
                        </p>
    ...
    <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
      <div className="mb-4">
        ...
        <p className="text-gray-400 text-sm mt-1">
          Found {suggestion.suggestedMatches.length} possible{" "}
          {suggestion.suggestedMatches.length === 1 ? "match" : "matches"}
        </p>
      </div>
